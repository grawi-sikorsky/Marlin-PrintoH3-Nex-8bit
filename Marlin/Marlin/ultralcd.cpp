...
uint16_t max_display_update_time = 0;
  ...
  void PrintPanicEEPROM(); // dodane
  void ResumePrintH3(); // dodane
...
void kill_screen(const char* lcd_msg) {
  ...
  void lcd_completion_feedback(const bool good/*=true*/) {
	  if (good) {
		  lcd_buzz(70, 2300); // dodane beeper git
		  lcd_buzz(70, 2900); // dodane beeper git
	  }
	  else {
		  lcd_buzz(120, 700); // dodane beeper lipka
		  lcd_buzz(120, 000);
		  lcd_buzz(120, 700);
	  }
  }
   ...
   */
...
#ifdef DEBUG_VLCS
	void showbabystep_done()
	{
		_babystep_z_shift = eeprom_read_dword((uint32_t*)EEPROM_PANIC_BABYSTEP_Z);

		START_MENU();
		MENU_BACK(MSG_WATCH);
		STATIC_ITEM("baby: ", false, false, ftostr43sign(_babystep_z_shift));
		STATIC_ITEM("baby: ", false, false, ftostr43sign(planner.steps_to_mm[Z_AXIS] * _babystep_z_shift));
		STATIC_ITEM("baby-ep: ", false, false, ftostr43sign(_babystep_z_shift));
		MYSERIAL.print("baby: ");
		MYSERIAL.println(_babystep_z_shift);
		MYSERIAL.print("baby: ");
		MYSERIAL.println(planner.steps_to_mm[Z_AXIS] * _babystep_z_shift);
		MYSERIAL.print("baby-epprom: ");
		MYSERIAL.println(eeprom_read_dword((uint32_t*)EEPROM_PANIC_BABYSTEP_Z));
		END_MENU();
	}
	void baby_save() {
	...
	void baby_reset() {
		eeprom_update_dword((uint32_t*)(EEPROM_PANIC_BABYSTEP_Z), 0);
	}
	...
	void print_eeprom()
	{
		MYSERIAL.print("baby-epprom: ");
		MYSERIAL.println(eeprom_read_dword((uint32_t*)EEPROM_PANIC_BABYSTEP_Z));
		MYSERIAL.print("fail count: ");
		MYSERIAL.println(eeprom_read_byte((uint8_t*)EEPROM_PANIC_POWER_FAIL_COUNT));
		MYSERIAL.print("sdpos: ");
		MYSERIAL.println(eeprom_read_dword((uint32_t*)EEPROM_PANIC_SD_FILE_POSITION));
		MYSERIAL.print("x: ");
		MYSERIAL.println(eeprom_read_float((float*)EEPROM_PANIC_CURRENT_XPOS));
		MYSERIAL.print("y: ");
		MYSERIAL.println(eeprom_read_float((float*)EEPROM_PANIC_CURRENT_YPOS));
		MYSERIAL.print("z: ");
		MYSERIAL.println(eeprom_read_float((float*)EEPROM_PANIC_CURRENT_ZPOS));
		MYSERIAL.print("e: ");
		MYSERIAL.println(eeprom_read_float((float*)EEPROM_PANIC_CURRENT_EPOS));
		MYSERIAL.print("axis: ");
		MYSERIAL.println(eeprom_read_byte((uint8_t*)EEPROM_PANIC_AXIS_REL_MODES));
		MYSERIAL.print("hot: ");
		MYSERIAL.println(eeprom_read_word((uint16_t*)EEPROM_PANIC_TARGET_HOTEND));
		MYSERIAL.print("bed: ");
		MYSERIAL.println(eeprom_read_word((uint16_t*)EEPROM_PANIC_TARGET_BED));
		MYSERIAL.print("fr: ");
		MYSERIAL.println(eeprom_read_word((uint16_t*)EEPROM_PANIC_FEEDRATE));
		MYSERIAL.print("fan: ");
		MYSERIAL.println(eeprom_read_word((uint16_t*)EEPROM_PANIC_FAN_SPEED));
		MYSERIAL.print("sdname: ");
		MYSERIAL.println(eeprom_read_byte((uint8_t*)EEPROM_SD_FILENAME));
	}
  ...
  void lcd_main_menu() {
	...
	#ifdef DEBUG_VLCS
	  MENU_ITEM(submenu, "babystep done", showbabystep_done);
	  MENU_ITEM(function, "print eprom", print_eeprom);
	  MENU_ITEM(function, "baby save", baby_save);
	  MENU_ITEM(function, "baby reset", baby_reset);
	#endif
   ...
   */
    ...
    void _lcd_babystep(const AxisEnum axis, const char* msg) {
      ...
      void lcd_babystep_z() { lcd_goto_screen(_lcd_babystep_z); babysteps_done = 0; defer_return_to_status = true; } // dodane 
  ...
  void watch_temp_callback_bed() {
    ...
    void lcd_enqueue_filament_change() {
      ...
      #endif
		if ((axis_homed[X_AXIS] && axis_homed[Y_AXIS] && axis_homed[Z_AXIS]) == false)  //dodane caly if
		{
  ...
  void lcd_tune_menu() {
    ...
    //
    MENU_BACK(MSG_MAIN);
	//MENU_ITEM(gcode, "Panic", PSTR("M811")); // dodane debug
	#ifdef DEBUG_VLCS
	MENU_ITEM(function, "babyyy", showbabystep_done);
	#endif
    //
    // Speed:
    //
  ...
  void lcd_cooldown() {
    ...
    static void lcd_store_settings()   { lcd_completion_feedback(settings.save()); } // dodane beeper
    static void lcd_load_settings()    { lcd_completion_feedback(settings.load()); } // dodane beeper
  #endif
    ...
    void _lcd_level_bed_continue() {
      ...
      enqueue_and_echo_commands_P(PSTR("G28"));
	  set_bed_leveling_enabled(false); // wylacza bed leveling przed probkowaniem
    ...
    void lcd_bed_leveling() {
      ...
      if (!(axis_known_position[X_AXIS] && axis_known_position[Y_AXIS] && axis_known_position[Z_AXIS]))
		  MENU_ITEM(gcode, MSG_AUTO_HOME, PSTR("G28"));  //dodane, bylo: PSTR("G28 X");
	...
	// DODANE albowiem ** stwierdzily ze bazowanie G28 musi wylaczac leveling mode M420 S1
  ...
  void lcd_prepare_menu() {
	...
	MENU_ITEM(function, MSG_AUTO_HOME, bazowanie_bed_on);  //dodane, bylo: PSTR("G28 X");
	  ...
	  //dodane
	  //MENU_ITEM(gcode, "Panic", PSTR("M811"));
	  //MENU_ITEM(function, "Odczyt Panic EEPROM", PrintPanicEEPROM);
  ...
  void lcd_control_menu() {
    ...
    //MENU_ITEM(submenu, MSG_FILAMENT, lcd_control_filament_menu); //usuniete.
   ...
   */
    ...
    void _lcd_autotune(int16_t e) {
	  ...
	  lcd_buzz(70, 2300);//dodane
	  lcd_buzz(70, 3100);//dodane
	  delayMicroseconds(140);
	  lcd_return_to_status();//dodane
  ...
  */
   ...
   */
    ...
    void lcd_sdcard_menu() {
      ...
      void lcd_info_stats_menu() {
		...
		STATIC_ITEM(MSG_INFO_STATS_MENU ": ", false, true);
        STATIC_ITEM(MSG_INFO_PRINT_COUNT ": ", false, false, itostr3left(stats.totalPrints));          // Print Count: 999
        STATIC_ITEM(MSG_INFO_COMPLETED_PRINTS": ", false, false, itostr3left(stats.finishedPrints));   // Completed  : 666

		#if ENABLED(PLOSS_SUPPORT)
		STATIC_ITEM(MSG_INFO_VLCS_FAILED_PRINTS": ", false, false, itostr3left(eeprom_read_byte((uint8_t*)EEPROM_PANIC_POWER_FAIL_COUNT))); // dodane power fail count
		#endif

        duration_t elapsed = stats.printTime;
        elapsed.toString(buffer);

        STATIC_ITEM(MSG_INFO_PRINT_TIME ": ", false, true);                                            // Total print Time:
    ...
    void lcd_info_thermistors_menu() {
      ...
      STATIC_ITEM(MSG_INFO_MAX_TEMP ": " STRINGIFY(HEATER_0_MAXTEMP), false);
        ...
        #include "thermistornames.h"
        STATIC_ITEM("T-Bed:" THERMISTOR_NAME, false, true);
    ...
    void lcd_info_board_menu() {
      ...
      #if POWER_SUPPLY == 0
        STATIC_ITEM(MSG_INFO_PSU ": Generic 24V", true);
      #elif POWER_SUPPLY == 1
    ...
    void lcd_info_printer_menu() {
      if (lcd_clicked) { return lcd_goto_previous_menu(); }
      START_SCREEN();
      STATIC_ITEM(MSG_MARLIN " v. " SHORT_BUILD_VERSION, true, true);  // Marlin  x.x.x-Branch
	  ...
	  #endif
	  #if ENABLED(PLOSS_SUPPORT)
		STATIC_ITEM(MSG_INFO_VLCS_SUPPORT);
	  #endif
	  #if ENABLED(FILAMENT_RUNOUT_SENSOR)
		STATIC_ITEM(MSG_INFO_FILAMENT_SENSOR_SUPPORT);
	  #endif
		STATIC_ITEM("Wsparcie: ", true);
		STATIC_ITEM("support@printo3d.pl", true);
      END_SCREEN();
    }

	/**
	*
	* About Printer > Printo3D info
	*
	*/
	void lcd_info_printo3d_menu() {
		if (lcd_clicked) { return lcd_goto_previous_menu(); }
		START_SCREEN();
		STATIC_ITEM(MSG_INFO_P3D_1, true, false);
		STATIC_ITEM(MSG_INFO_P3D_2, true, false);
		STATIC_ITEM(MSG_INFO_P3D_3, true, false);
		STATIC_ITEM(MSG_INFO_P3D_MAIL, true, false);
		STATIC_ITEM(MSG_INFO_P3D_PHONE, true, false);
		END_SCREEN();
	}
    ...
    void lcd_info_menu() {
      ...
      //MENU_ITEM(submenu, MSG_INFO_THERMISTOR_MENU, lcd_info_thermistors_menu); // Thermistors > //usuniete
      #if ENABLED(PRINTCOUNTER)
        MENU_ITEM(submenu, MSG_INFO_STATS_MENU, lcd_info_stats_menu);          // Printer Statistics >
      #endif
		MENU_ITEM(submenu, MSG_INFO_PRINTO3D, lcd_info_printo3d_menu);
      END_MENU();
    }
  #endif // LCD_INFO_MENU
   ...
   */
	...
	void lcd_ploss_menu_ask_twice() {
		START_MENU();
		STATIC_ITEM("Jesteœ pewien?", true, true);
		STATIC_ITEM("");
		MENU_ITEM(function, "Tak", lcd_return_to_status);
		MENU_ITEM(function, "Nie", ploss_recovery_menu);
		END_MENU();
	}
	...
	void ploss_recovery_menu() {
		START_MENU();
		STATIC_ITEM("Zanik zasilania", true, true);
	...
	void ploss_recovery_menu_resuming() {
		START_SCREEN();
			STATIC_ITEM("");
			STATIC_ITEM("Wznawianie wydruku", true, false);
			STATIC_ITEM("po zaniku zasilania", true, false);
		END_SCREEN();
		if (lcd_clicked) {
			lcd_return_to_status();
		}
	...
	void ploss_recovery_menu_last_confirm() {
		START_SCREEN();
		STATIC_ITEM("");
		STATIC_ITEM("Usun nadmiar", true, false);
		STATIC_ITEM("filamentu i kliknij", true, false);
		STATIC_ITEM("aby wznowic wydruk", true, false);
		END_SCREEN();
		if (lcd_clicked) {
			lcd_ploss_menu_response = PLOSS_LCD_RESPONSE_YES;
		}
	...
	void ploss_recovery_menu_no_resume() {
		START_SCREEN();
			STATIC_ITEM("");
			STATIC_ITEM("Uruchamianie", true, false);
			STATIC_ITEM("po zaniku zasilania", true, false); 
			STATIC_ITEM("Bazowanie osi", true, false);
		END_SCREEN();
		if (lcd_clicked) {
			lcd_return_to_status();
		}
...
//
	...
	void lcd_ploss_recovery_menu(const PlossMenuMessage message) {
		...
		case PLOSS_LCD_MANUAL_RECOVERY:
			...
			lcd_goto_screen(ploss_recovery_menu);
			break;
		case PLOSS_LCD_RECOVERY_RESUMING:
			defer_return_to_status = false;
			lcd_goto_screen(ploss_recovery_menu_resuming);
			break;
		case PLOSS_LCD_MENU_NO_RESUME:
			defer_return_to_status = false;
			lcd_goto_screen(ploss_recovery_menu_no_resume);
			enqueue_and_echo_commands_P(PSTR("G28"));
			break;
		case PLOSS_LCD_MENU_LAST_CONFIRM:
			defer_return_to_status = true;
			lcd_ploss_menu_response = PLOSS_LCD_RESPONSE_WAIT_FOR_LAST_CONFIRMATION;
			lcd_goto_screen(ploss_recovery_menu_last_confirm);
			break;
  ...
  /**
  * 	Zapisuje dodatkowo w EEPROM nazwe pliku oraz sciezke dostepu
  */
    void menu_action_sdfile(const char* filename, char* longFilename) { // dodane printo h3 
...
void lcd_init() {
	/*
	* DODANE Z RACJI LIPNEGO RESET_PINU NA NIEKTORYCH OLEDACH
	*/
	int LCD_Reset = 29;
	int LCD_Reset_state;
  ...
  lcd_buzz(100, 2300); // dodane - wejsciowy brzeczyk
  lcd_buzz(100, 2600);
  lcd_buzz(100, 3100);
...
void lcd_update() {
    ...
    if (sd_status != lcd_sd_status && lcd_detected()) {
      ...
      if (sd_status) {
		...
		#if defined PRINTOH3_SPEAKER_SOUNDS // dodane
		lcd_buzz(50, 2300); // dodane - wejsciowy brzeczyk
		lcd_buzz(50, 3100);
		#endif
      }
      else {
		...
		#if defined PRINTOH3_SPEAKER_SOUNDS // dodane
		lcd_buzz(50, 3100); // dodane - wejsciowy brzeczyk
		lcd_buzz(50, 2300);
		#endif
      }
