...
#if FAN_COUNT > 0
  uint16_t fanSpeeds[FAN_COUNT] = { 0 }; //dodane uint8_t zamiast16
  #if ENABLED(PROBING_FANS_OFF)
    bool fans_paused = false;
    uint16_t paused_fanSpeeds[FAN_COUNT] = { 0 }; //dodane uint8_t zamiast16
  #endif
#endif
...
inline void gcode_M121() { endstops.enable_globally(false); }
  ...
  inline void gcode_M125() {
    ...
    if (pause_print(retract, z_lift, x_pos, y_pos, 0)) { // wartosc 0 = pluj, czyli zdefiniowana wczesniej wartosc ekstruzji przy zmianie filamentu m600
...
inline void gcode_M812() {
  ...
  inline void gcode_M600() {
	...
	if (pause_print(retract, z_lift, x_pos, y_pos, pluj, unload_length, beep_count, true)) {
...
#endif // FILAMENT_RUNOUT_SENSOR  do przerobienia czujka
