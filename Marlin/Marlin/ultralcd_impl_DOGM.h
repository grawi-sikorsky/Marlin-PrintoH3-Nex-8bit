...
static void lcd_implementation_status_screen() {
  ...
  if (PAGE_UNDER(STATUS_SCREENHEIGHT + 1)) {
	  //const int16_t x = random(0, 1000);
	  u8g.drawBitmapP(94, 1, 1, 8,
		  blink && fanSpeeds[0] ? status_fan1_bmp : status_fan2_bmp
	  );
  ...
  if (PAGE_UNDER(28)) {
    ...
    #if HAS_FAN0
        // Fan
        const int16_t per = ((fanSpeeds[0] + 1) * 100) / 256; //dodane uint8_t   stare: const int16_t per = ((fanSpeeds[0] + 1) * 100) / 256;
        if (per) {
          u8g.setPrintPos(104, 9);
...
#define PROGRESS_BAR_WIDTH (LCD_PIXEL_WIDTH - PROGRESS_BAR_X)
  ...
  */ //dodane
  ...
  if (PAGE_CONTAINS(51 - INFO_FONT_HEIGHT, 49)) {
	...
	u8g.drawBitmapP(0, 42, 2, 9, status_feedrate_bmp);
