...
#if FAN_COUNT > 0
  extern uint16_t fanSpeeds[FAN_COUNT]; // dodane uint8_t zamaist 16
  #if ENABLED(PROBING_FANS_OFF)
    extern bool fans_paused;
    extern uint16_t paused_fanSpeeds[FAN_COUNT]; // dodane uint8_t zamaist 16
  #endif
#endif
...
#if ENABLED(Z_PROBE_ALLEN_KEY) || ENABLED(Z_PROBE_SLED) || HAS_PROBING_PROCEDURE || HOTENDS > 1 || ENABLED(NOZZLE_CLEAN_FEATURE) || ENABLED(NOZZLE_PARK_FEATURE) || ENABLED(HOME_BEFORE_FILAMENT_CHANGE)
